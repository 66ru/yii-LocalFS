...
class Image_ImageMagick_Driver extends Image_Driver {
	...
	public function cropZoom($properties)
	{
		if (empty($properties['location']))
		{
			$properties['location'] = 'center';
		}
		...
		if ($error = exec(escapeshellcmd($this->dir.'convert'.$this->ext).' -gravity ' . $properties['location'] .' -extent ' . $dim . ' '.$this->cmd_image.' '.$this->cmd_image))
		{
			$this->errors[] = $error;
			return FALSE;
		}
