...
Yii::import('localFS.components.image.Image_Driver');
...
class Image {
	...
	public function __construct($image, $config = NULL)
	{
        ...
        // Load the driver
		Yii::import("localFS.components.image.drivers.$driver");
	...
	public function crop($width, $height, $top = 'center', $left = 'center')
	{
		...
		if ( ! $this->valid_size('top', $top))
			throw new CException('image invalid top');

		if ( ! $this->valid_size('left', $left))
			throw new CException('image invalid left');
	...
	public function cropZoom($width, $height, $location = 'center')
	{
		if ( ! $this->valid_size('width', $width))
			throw new CException('image invalid width');
		...
		if ( ! $this->valid_size('location', $location))
			throw new CException('image invalid location');

		if (empty($width) AND empty($height))
			throw new CException('image invalid dimensions');

		$this->actions['cropZoom'] = array
		(
			'width'    => $width,
			'height'   => $height,
			'location' => $location,
